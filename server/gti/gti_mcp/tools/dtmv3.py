...
import asyncio
import re
import time # Import time for measuring intervals
...
_VT_API_LOCK = asyncio.Lock()
_VT_API_LAST_REQUEST_TIME = 0.0 # Stores timestamp of the last *completed* DTM API request
# Configure your desired minimum interval between requests (e.g., 1.0 for 1 request/second)
# Adjust this based on the actual DTM API rate limits.
_VT_API_MIN_INTERVAL = 1.0 # seconds

async def _throttled_api_call(api_client, api_path: str, params: typing.Dict[str, typing.Any]):
    """
    Helper function to make a throttled API call.
    Ensures a minimum interval between consecutive DTM API requests.
    """
    async with _VT_API_LOCK:
        global _VT_API_LAST_REQUEST_TIME
        current_time = time.time()
        elapsed_since_last_call = current_time - _VT_API_LAST_REQUEST_TIME

        if elapsed_since_last_call < _VT_API_MIN_INTERVAL:
            wait_time = _VT_API_MIN_INTERVAL - elapsed_since_last_call
            logger.debug(f"Throttling DTM API request. Waiting for {wait_time:.2f} seconds.")
        ...
        try:
            ...
            # Update the last request time after the API call completes (or at least returns)
            _VT_API_LAST_REQUEST_TIME = time.time()
...
async def list_dtm_alerts(
    ...
    try:
        # Fixed logger.info f-string
        logger.info(f"Requesting DTM alerts. Path: {api_path}, Params: {params}")
        api_client = vt_client(ctx)
        
        # --- MODIFICATION: Use throttled API call ---
        response = await _throttled_api_call(api_client, api_path, params)
        # --- END MODIFICATION ---
...
async def list_child_alerts_for_aggregate_bucket(
    ...
    (Full docstring as per previous correct version)
    """
    api_path = f"/dtm/alerts/{aggregate_id}/aggregates" # Fixed f-string usage
    params: typing.Dict[str, typing.Any] = {}
    ...
    if cursor:
        ...
        if include_monitor_name is not None: params["monitor_name"] = str(include_monitor_name).lower()
        if has_analysis is not None: params["has_analysis"] = str(has_analysis).lower()
        params["buckets"] = "false" # This parameter is explicitly set to false for child alerts
        if since_date: params["since"] = since_date
        if until_date: params["until"] = until_date
        if truncate_length is not None: params["truncate"] = truncate_length
        if statuses: params["status"] = statuses
    ...
    try:
        ...
        if isinstance(api_response_data, dict):
            ...
            # For consistency with the general /dtm/alerts endpoint, we assume "alerts".
            data_content = api_response_data.get("alerts", [])
            # --- END MODIFICATION ---
